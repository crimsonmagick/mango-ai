...
class CompletionConversationSerializerTest extends Specification {
    ...
    def "ParseConversation - expressions should be separated by a newline"() {
        ...
        final expressions = [new ExpressionValue("You are a chatbot.", SYSTEM),
                             new ExpressionValue("Hello, how can I help you today fine sir and/or madame?", PAL),
                             new ExpressionValue("How do replace the batteries in my remote?", USER),
                             new ExpressionValue("Take off the backcover. Then remove the batteries for safe disposal. Insert the new batteries, and put the cover back on.", PAL)]
        final conversation = new ConversationEntity("1234", expressions)
